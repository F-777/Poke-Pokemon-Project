...
const Paimon = () => {
  ...
  const fetchRecommendedPokemon = async () => {
    const recommendedIds = Array.from({ length: 250 }, (_, i) => i + 1); // added 150 pokemon
  ...
  const searchPokemon = async () => {
    try {
      setError('');
      const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${query.toUpperCase()}`);
  ...
  return (
    <div className="app">
      ...
      <div className="header" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '2rem'}}>
        <img src="public/pokemon.png" alt="pokemon" className='logos' style={{ width: '400px' , height: '150px' }} />
